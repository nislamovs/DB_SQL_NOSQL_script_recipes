...
SELECT
    ...
    COALESCE(pup.firstname, 'No') AS Pupil_firstname,
    COALESCE(pup.lastname, 'children')  AS Pupil_lastname,
    COALESCE(pup.birthdate::TEXT, 'No') AS Pupil_birthdate,
    COALESCE((CASE
...
SELECT
   ...
   SELECT CASE
            WHEN AVG(m.value) >= 4 THEN 'Passed'
            WHEN AVG(m.value) < 4 THEN 'Failed'
          END AS Status
