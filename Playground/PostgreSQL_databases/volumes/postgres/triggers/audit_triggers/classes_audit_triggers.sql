CREATE FUNCTION classes_insert_trigger_function()
    RETURNS TRIGGER
    LANGUAGE PLPGSQL
AS $$
    ...
    DECLARE tablename VARCHAR(10) := 'classes';
...
CREATE TRIGGER classes_insert_trigger AFTER INSERT
ON classes
FOR EACH ROW EXECUTE PROCEDURE classes_insert_trigger_function();

-----------------------------------------------------------------------------------------------------------------------

CREATE FUNCTION classes_update_trigger_function()
    RETURNS TRIGGER
    LANGUAGE PLPGSQL
AS $$
    DECLARE recordid INTEGER := NEW.id;
...
CREATE TRIGGER classes_update_trigger AFTER UPDATE
ON classes
FOR EACH ROW EXECUTE PROCEDURE classes_update_trigger_function();

-----------------------------------------------------------------------------------------------------------------------

CREATE FUNCTION classes_delete_trigger_function()
    RETURNS TRIGGER
    LANGUAGE PLPGSQL
AS $$
...
CREATE TRIGGER classes_delete_trigger AFTER DELETE
ON classes
FOR EACH ROW EXECUTE PROCEDURE classes_delete_trigger_function();
